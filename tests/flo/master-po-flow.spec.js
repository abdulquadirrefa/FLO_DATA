...
console.log(`[Step5] Found ${rowCount} component rows.`);
      ...
      if (rowCount === 4) {
        ...
        await dataRows.nth(1).getByRole('combobox').click();
        await page.getByRole('option', { name: 'CG1' }).last().click();

        await dataRows.nth(2).getByRole('combobox').click();
        await page.getByRole('option', { name: 'CG2' }).last().click();

        await dataRows.nth(3).getByRole('combobox').click();
        await page.getByRole('option', { name: 'CG2' }).last().click();
        ...
        throw new Error(`[Step5] Unexpected row count: ${rowCount}. Expected 3 or 4 rows.`);
      }
